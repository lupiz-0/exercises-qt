...
std::ostream& operator<<(std::ostream& ostream, const Date& date) {
  ostream << std::setfill('0') << std::setw(2) << date.day() << '/'
...
bool operator==(const Date& dateA, const Date& dateB) {
  return dateA.day() == dateB.day() && dateA.month() == dateB.month() &&
...
Date Date::fromString(const std::string& string) {
  ...
  if (!Date::isInteger(dayString) || !Date::isInteger(monthString) ||
      !Date::isInteger(yearString))
    return Date();
